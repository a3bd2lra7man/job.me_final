...
class ExperiencesUpdater {
  final API _api;
  String _sessionId = DateTime.now().millisecondsSinceEpoch.toString();
  bool _isLoading = false;

  ExperiencesUpdater() : _api = API();

  Future updateExperience(Experience experience) async {
    var url = ExperienceUrls.updateExperienceUrls(experience.id!);
    _sessionId = DateTime.now().millisecondsSinceEpoch.toString();
    var apiRequest = APIRequest.withId(url, _sessionId);
    apiRequest.addParameters(experience.toJson());
    _isLoading = true;
    try {
