...
class CompanyDepartmentAdder {
  final API _api;
  String _sessionId = DateTime.now().millisecondsSinceEpoch.toString();
  bool _isLoading = false;

  CompanyDepartmentAdder() : _api = API();

  Future addCompanyDepartment(int companyId,CompanyDepartment companyDepartment) async {
    var url = CompanyDepartmentsUrls.addCompanyDepartmentUrl();
    _sessionId = DateTime.now().millisecondsSinceEpoch.toString();
    var apiRequest = APIRequest.withId(url, _sessionId);
    var map = companyDepartment.toJson();
