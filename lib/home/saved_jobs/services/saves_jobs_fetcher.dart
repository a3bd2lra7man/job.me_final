...
import 'package:job_me/home/saved_jobs/constants/saved_jobs_urls.dart';


class SavedJobsFetcher {
  final API _api;
  ...
  SavedJobsFetcher() : _api = API();

  Future<List<JobAdvertisement>> getNextSavedJobs() async {
    var url = SavedJobsUrls.savedJobsUrls(_currentPage);
  ...
  Future<List<JobAdvertisement>> _processResponse(APIResponse apiResponse) async {
    ...
    var savedJobs = <JobAdvertisement>[];
    try {
      for (var element in responseList) {
        savedJobs.add(JobAdvertisement.fromJson(element));
      }
      _updatePaginationData(savedJobs.length);
      return savedJobs;
    } catch (e) {
